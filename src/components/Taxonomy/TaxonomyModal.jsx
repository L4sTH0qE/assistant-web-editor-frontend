...
import { Modal, Table, Button, Form, Input, Space, Popconfirm, message, Typography, Tabs, Tag } from 'antd';
...
export const TaxonomyModal = ({ isOpen, onClose }) => {
    ...
    const columns = [
        {
            ...
            dataIndex: 'name',
            key: 'name',
            render: (text) => <Tag color={activeTab === 'rubric' ? 'blue' : activeTab === 'tag' ? 'green' : 'orange'}>{text}</Tag>,
        ...
        },
        {
            ...
            width: 100,
            render: (_, record) => (
                <Popconfirm title="Удалить из справочника?" onConfirm={() => handleDelete(record.id)} okText="Да" cancelText="Нет">
    ...
    const renderForm = (placeholder) => (
        <Form form={form} layout="inline" onFinish={handleAdd} style={{ marginBottom: 20, padding: 16, background: '#fafafa', borderRadius: 6 }}>
    ...
    return (
        <Modal
            title={<Typography.Title level={4} style={{fontFamily: 'HSE Sans', margin: 0}}>Справочник метаданных</Typography.Title>}
            open={isOpen}
            onCancel={onClose}
            footer={null}
            width={700}
        >
            <Tabs defaultActiveKey="rubric" onChange={(key) => setActiveTab(key)}>
